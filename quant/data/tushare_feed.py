...
from .feeds.tushare import TushareSource

def _fetch_daily(symbol: str, start: str, end: str) -> pd.DataFrame:
    """Fetch daily bars from Tushare pro and normalise columns."""
    logger.info(f"Fetching {symbol} from Tushare ({start} ~ {end})")
    return TushareSource().fetch_daily(symbol, start, end)


class TuShareFeed(DataFeed):
    """Daily-bar data feed backed by Tushare Pro with Parquet caching."""
    ...
    def subscribe(self, symbols: list[str]) -> None:
        ...
        if missing:
            frames = TushareSource().fetch_daily_many(
                missing,
                self.start_date.replace("-", ""),
                self.end_date.replace("-", ""),
            )
            for sym in missing:
                df = frames.get(sym, pd.DataFrame())
                if self.use_cache:
                    save_cache(sym, df)
                self._data[sym] = df.reset_index(drop=True)
                logger.info(f"Subscribed {sym}: {len(df)} bars")
